...
import { getRideById, updateRide, calculatePrice } from '../../utils/rideService';
...
const EditRide = () => {
  ...
  useEffect(() => {
    ...
    if (ride.driverId !== currentUser.id) {
      showError('You can only edit your own rides');
      navigate('/driver/dashboard');
      return;
    }
    ...
    const localDateTime = new Date(departureDate.getTime() - departureDate.getTimezoneOffset() * 60000)
      .toISOString()
      .slice(0, 16);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      updateRide(id, user.id, formData);
      ...
      showError(err.message || 'Failed to update ride. Please try again.');
    } finally {
      setLoading(false);
    }
  };
